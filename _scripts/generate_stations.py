...
def render_similar_section(s):
    ...
    for sim in similar:
        ...
        sim_local = os.path.join(root_dir, 'img', f'{sim_slug}-montagne.jpg')
        if os.path.exists(sim_local):
            photo = f'../img/{sim_slug}-montagne.jpg'
        else:
...
def render_page(s):
    ...
    local_photo_path = os.path.join(root_dir, 'img', f'{slug}-montagne.jpg')
    if os.path.exists(local_photo_path):
        photo = f'../img/{slug}-montagne.jpg'
    else:
        photo = s.get('photo') or 'https://images.unsplash.com/photo-1551524559-8af4e6624178?w=1200&q=80'
    local_station_path = os.path.join(root_dir, 'img', f'{slug}-station.jpg')
    photo_station = f'../img/{slug}-station.jpg' if os.path.exists(local_station_path) else None
  ...
  <div class="grid-2">
    <div>
      <div class="card">
        ...
        {f'<img src="{photo_station}" alt="{s["name"]} pistes de ski" style="width:100%;height:160px;object-fit:cover;border-radius:8px;margin-bottom:12px" loading="lazy">' if photo_station else ''}
