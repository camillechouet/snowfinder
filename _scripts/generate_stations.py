...
<head>
  ...
  <style>
    *{{box-sizing:border-box;margin:0;padding:0}}
    ...
    .official-btn{{
      width:100%;border:2px solid var(--wood-light);background:var(--wood-pale);
      border-radius:var(--radius);padding:14px 16px;cursor:pointer;
      text-align:left;font-family:"DM Sans",sans-serif;font-size:.88rem;
      font-weight:700;color:var(--text-mid);margin-bottom:16px;
      transition:background .15s,border-color .15s;line-height:1.3
    }}
    .official-btn:hover{{background:var(--wood-light);border-color:var(--wood)}}
  ...
  <div class="main-grid">
    ...
    <div class="sidebar">
      ...
      <button onclick="showOfficialSheet()" class="official-btn">
        🎿 Site officiel de {s['name']}
        <span style="font-size:.72rem;opacity:.7;display:block;margin-top:2px">Pistes · Forfaits · Webcams</span>
      </button>
...
<div id="officialSheet" style="display:none;position:fixed;bottom:0;left:0;right:0;z-index:901;background:white;border-radius:22px 22px 0 0;padding:28px 24px 36px;box-shadow:0 -8px 40px rgba(0,0,0,.2);transform:translateY(100%);transition:transform .35s cubic-bezier(.32,.72,0,1)">
  ...
  <a id="officialLink" href="https://www.google.com/search?q={s['name'].replace(' ', '+')}+station+ski+site+officiel" target="_blank" rel="noopener" onclick="hideOfficialSheet()" style="display:block;text-align:center;font-size:.8rem;color:#8a7060;text-decoration:underline;padding:8px">
