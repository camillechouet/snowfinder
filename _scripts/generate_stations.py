...
(function loadMeteo(){{
  ...
  fetch(url).then(function(r){{return r.json();}}).then(function(d){{
    ...
    if(d.current){{
      ...
      // Onglet Enneigement — village
      var villageEl=document.getElementById('snowVillageNow');
      if(villageEl){{
        if(sd!=null&&sd>0){{
          var villageCm=Math.round(sd*100);
          villageEl.textContent=villageCm;
        }}else{{
          villageEl.textContent='0';
        }}
      }}
      // Onglet Enneigement — sommet (correction altitudinale : +30% par 1000m supplémentaire)
      var sommetEl=document.getElementById('snowSommetNow');
      if(sommetEl){{
        var altDiff=({s['alt_max']}-{s['alt_min']})/1000;
        var factor=1+Math.min(0.8,altDiff*0.35);
        if(sd!=null&&sd>0){{
          var sommetCm=Math.round(sd*100*factor);
          sommetEl.textContent=sommetCm;
        }}else{{
          sommetEl.textContent='0';
        }}
      }}
    }}
  ...
  <div class="main-grid">
    ...
    <div>
      ...
      <div class="tab-content" id="tab-neige">
        ...
        <!-- Tableau comparatif village / sommet -->
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px" id="snowCompareGrid">
          <!-- Village -->
          <div style="background:linear-gradient(135deg,#e8f3fb,#d0e7f5);border-radius:12px;padding:16px;text-align:center">
            <div style="font-size:.68rem;font-weight:800;text-transform:uppercase;letter-spacing:.07em;color:#3a7db8;margin-bottom:8px">🏘 Village · {s['alt_min']} m</div>
            <div id="snowVillageNow" style="font-family:'DM Serif Display',serif;font-size:2.2rem;color:#1a5a8a;line-height:1">—</div>
            <div style="font-size:.72rem;color:#3a7db8;font-weight:600;margin-top:3px">cm actuellement</div>
            <div style="margin-top:10px;padding-top:10px;border-top:1px solid rgba(26,90,138,.15)">
              <div style="font-size:.65rem;color:#5a8ab8;font-weight:700;text-transform:uppercase;margin-bottom:4px">Référence saison</div>
              <div style="font-size:1.1rem;font-weight:800;color:#1a5a8a">{snow_ref_village} cm</div>
            </div>
          </div>
          <!-- Sommet -->
          <div style="background:linear-gradient(135deg,#1a5a8a,#2a7ab8);border-radius:12px;padding:16px;text-align:center">
            <div style="font-size:.68rem;font-weight:800;text-transform:uppercase;letter-spacing:.07em;color:rgba(255,255,255,.75);margin-bottom:8px">⛰ Sommet · {s['alt_max']} m</div>
            <div id="snowSommetNow" style="font-family:'DM Serif Display',serif;font-size:2.2rem;color:white;line-height:1">—</div>
            <div style="font-size:.72rem;color:rgba(255,255,255,.75);font-weight:600;margin-top:3px">cm actuellement</div>
            <div style="margin-top:10px;padding-top:10px;border-top:1px solid rgba(255,255,255,.2)">
              <div style="font-size:.65rem;color:rgba(255,255,255,.6);font-weight:700;text-transform:uppercase;margin-bottom:4px">Référence saison</div>
              <div style="font-size:1.1rem;font-weight:800;color:white">{snow_ref_sommet} cm</div>
            </div>
          </div>
        </div>
        ...
        <div style="background:white;border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:14px">
          <div style="font-size:.72rem;font-weight:800;text-transform:uppercase;letter-spacing:.07em;color:var(--text-light);margin-bottom:10px">❄️ Chutes de neige prévues — 5 jours</div>
          <div id="snowForecastBars" style="display:flex;gap:8px;align-items:flex-end;height:70px">
            <div style="color:var(--text-light);font-size:.75rem;line-height:3">Chargement…</div>
          </div>
          <div id="snowForecastLabels" style="display:flex;gap:8px;margin-top:6px"></div>
        </div>

        <!-- Analyse fiabilité -->
        <div style="background:linear-gradient(135deg,#f8f5f0,#f0e8da);border:1px solid var(--wood-pale);border-radius:12px;padding:16px;margin-bottom:14px">
          <div style="font-size:.72rem;font-weight:800;text-transform:uppercase;letter-spacing:.07em;color:var(--text-light);margin-bottom:10px">📊 Analyse enneigement</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            <div>
              <div style="font-size:.7rem;font-weight:700;color:var(--text-light);margin-bottom:4px">Meilleure période</div>
              <div style="font-size:.88rem;font-weight:700;color:var(--text)">{'Déc → Avril' if s['alt_max'] >= 3000 else 'Jan → Mars' if s['alt_max'] >= 2500 else 'Jan → Fév' if s['alt_max'] >= 2000 else 'Janv uniquement'}</div>
            </div>
            <div>
              <div style="font-size:.7rem;font-weight:700;color:var(--text-light);margin-bottom:4px">Fiabilité</div>
              <div style="font-size:.88rem;font-weight:700;color:{'#2ea84e' if s['alt_max'] >= 2500 else '#f0a500' if s['alt_max'] >= 2000 else '#cc2200'}">{'Excellente ⭐⭐⭐' if s['alt_max'] >= 2500 else 'Bonne ⭐⭐' if s['alt_max'] >= 2000 else 'Moyenne ⭐'}</div>
            </div>
            <div>
              <div style="font-size:.7rem;font-weight:700;color:var(--text-light);margin-bottom:4px">Enneigement moy. saison</div>
              <div style="font-size:.88rem;font-weight:700;color:var(--text)">{s.get('snow', '—')} cm</div>
            </div>
            <div>
              <div style="font-size:.7rem;font-weight:700;color:var(--text-light);margin-bottom:4px">Altitude garantie neige</div>
              <div style="font-size:.88rem;font-weight:700;color:var(--text)">{'> 1 800 m ✓' if s['alt_min'] >= 1800 else '> 2 000 m ✓' if s['alt_max'] >= 2000 else '> 1 500 m ⚠️' if s['alt_max'] >= 1500 else 'Variable ⚠️'}</div>
            </div>
          </div>
        </div>

        <!-- Note explicative -->
        <p style="font-size:.72rem;color:var(--text-light);line-height:1.55">
          {snow_reliability} Les données en temps réel proviennent de l'API Open-Meteo. Les chiffres de référence saison sont des moyennes historiques indicatives.
        </p>
