...
{carousel_dots_html}
    ...
    if domaine:
        ...
        domaine_badge_html = f'''<a href="{domaine_url}" class="domaine-badge">
    🏔️ Fait partie du domaine <strong>{domaine['name']}</strong> — {domaine['km_total']} km reliés →
  </a>'''
        ...
        <h3 style="font-family:'DM Serif Display',serif;font-size:1.3rem;margin-bottom:10px">🏔️ {domaine['name']}</h3>
        <p style="color:var(--text-mid);margin-bottom:16px;line-height:1.6">{domaine['desc']}</p>
        ...
        {'<p style="font-size:.8rem;color:#a05a2c;background:#fff4e6;padding:8px 12px;border-radius:8px;margin:12px 0">⚠️ Liaison ouverte selon enneigement uniquement.</p>' if domaine.get('conditionnel') else ''}
        <a href="{domaine_url}" class="domaine-see-all">Voir la fiche complète du domaine →</a>
        {f'<h4 style="margin:20px 0 10px;font-size:1rem">Les autres stations du domaine</h4><div class="domaine-soeurs-grid">{soeurs_cards}</div>' if soeurs else ''}
      </div>'''
    else:
        domaine_badge_html = ''
        domaine_tab_btn = ''
...
<head>
  ...
  <style>
    *{{box-sizing:border-box;margin:0;padding:0}}
    ...
    /* DOMAINE SKIABLE RELIÉ */
    .domaine-badge{{
      display:flex;align-items:center;justify-content:center;gap:8px;
      background:linear-gradient(135deg,#0d3a6e 0%,#1a5a8a 100%);color:white;
      border-radius:14px;padding:13px 18px;margin:18px 0;text-decoration:none;
      font-weight:700;font-size:.88rem;text-align:center;transition:transform .15s,box-shadow .15s;
      box-shadow:0 4px 14px rgba(13,58,110,.25)
    }}
    .domaine-badge:hover{{transform:translateY(-1px);box-shadow:0 6px 20px rgba(13,58,110,.35)}}
    .domaine-stats-mini{{display:flex;gap:10px;flex-wrap:wrap;margin:14px 0}}
    .domaine-stats-mini div{{flex:1;min-width:90px;background:#f0f6fb;border-radius:10px;padding:10px 8px;text-align:center}}
    .domaine-stats-mini strong{{display:block;font-size:1.15rem;color:#0d3a6e;font-family:"DM Serif Display",serif}}
    .domaine-stats-mini span{{font-size:.68rem;color:var(--text-mid);text-transform:uppercase;letter-spacing:.04em}}
    .domaine-see-all{{display:inline-block;margin-top:6px;color:#1a5a8a;font-weight:700;text-decoration:none;font-size:.85rem}}
    .domaine-see-all:hover{{text-decoration:underline}}
    .domaine-soeurs-grid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:10px}}
    .domaine-soeur-card{{
      display:block;background:#f7efe2;border-radius:10px;padding:12px 14px;text-decoration:none;
      transition:transform .15s,box-shadow .15s
    }}
    .domaine-soeur-card:hover{{transform:translateY(-2px);box-shadow:0 4px 12px rgba(0,0,0,.1)}}
    .domaine-soeur-name{{font-weight:800;color:var(--text);font-size:.9rem}}
    .domaine-soeur-sub{{font-size:.72rem;color:#1a5a8a;font-weight:700;margin-top:2px}}
