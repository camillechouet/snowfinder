...
PLACEHOLDER_URLS = [
    ...
    "https://images.unsplash.com/photo-1465220183275-1faa863377e3?w=800&q=80",
    "https://images.unsplash.com/photo-1612450639537-d44bc045efc7?w=800&q=80",
    "https://images.unsplash.com/photo-1623057896740-99f8c2f090fc?w=800&q=80",
    "https://images.unsplash.com/photo-1614089916276-eb5be76854bd?w=800&q=80",
    "https://images.unsplash.com/photo-1485516195719-e9edaa29d2ba?w=800&q=80",
    "https://images.unsplash.com/photo-1654273102584-e48884efa8c5?w=800&q=80",
    "https://images.unsplash.com/photo-1629730441758-e9b5cb99550d?w=800&q=80",
    "https://images.unsplash.com/photo-1651135527891-da5b354b9f83?w=800&q=80",
    "https://images.unsplash.com/photo-1629730441874-8f1d1445bd2c?w=800&q=80",
    "https://images.unsplash.com/photo-1570396041540-e2bb67aa96c2?w=800&q=80",
    "https://images.unsplash.com/photo-1588932716990-60d57f5382dd?w=800&q=80",
    "https://images.unsplash.com/photo-1649280376955-8acd8deaa053?w=800&q=80",
    "https://images.unsplash.com/photo-1528659862616-22886eb53642?w=800&q=80",
    "https://images.unsplash.com/photo-1507534192483-69914c0692d7?w=800&q=80",
    "https://images.unsplash.com/photo-1547019122-0428a4d59174?w=800&q=80",
    "https://images.unsplash.com/photo-1613420365631-c588e7cac611?w=800&q=80",
    "https://images.unsplash.com/photo-1664436341001-b02974ae7524?w=800&q=80",
    "https://images.unsplash.com/photo-1562826542-449090f38c70?w=800&q=80",
]

def pick_placeholder(station_id, w=1200):
    """Sélection déterministe via ID station — même logique que le JS (s.id % 20).
    La même station aura toujours la même photo fallback sur la fiche et dans la grille."""
    idx = station_id % len(PLACEHOLDER_URLS)
    url = PLACEHOLDER_URLS[idx]
    # Les URLs ont déjà ?w=800&q=80, on remplace par la taille demandée
    return url.replace('?w=800&q=80', f'?w={w}&q=80')
...
def render_similar_section(s):
    ...
    for sim in similar:
        ...
        sim_photo_slug = to_photo_slug(sim['name'])
        sim_local = os.path.join(root_dir, 'img', f'{sim_photo_slug}1.jpg')
        if os.path.exists(sim_local):
            photo = f'../img/{sim_photo_slug}1.jpg'
        else:
            photo = pick_placeholder(sim['id'], w=400)
        ...
        <a href="{sim_slug}.html" style="display:block;border-radius:10px;overflow:hidden;background:white;box-shadow:0 2px 10px rgba(0,0,0,.08);text-decoration:none;transition:transform .2s,box-shadow .2s" onmouseover="this.style.transform='translateY(-3px)';this.style.boxShadow='0 6px 20px rgba(0,0,0,.12)'" onmouseout="this.style.transform='';this.style.boxShadow='0 2px 10px rgba(0,0,0,.08)'">
          <div style="position:relative;height:90px;overflow:hidden">
            <img src="{photo}" alt="{sim['name']}" style="width:100%;height:100%;object-fit:cover" loading="lazy" onerror="this.onerror=null;this.src='https://images.unsplash.com/photo-1454496522488-7a8e488e8606?w=400&q=80'">
...
def render_page(s):
    ...
    if all_photos:
        ...
        photo = pick_placeholder(s['id'], w=1200)
...
// ── MÉTÉO & ENNEIGEMENT (WorldWeatherOnline Ski API + Open-Meteo) ──
// WWO Ski API : données réelles par niveau (bas/milieu/sommet) + moyennes mensuelles historiques
// Open-Meteo : météo 5 jours (prévisions températures, icones, chutes de neige)
var WWO_KEY = 'VOTRE_CLE_WWO_ICI'; // ← Remplacer par votre clé WorldWeatherOnline
var WWO_LOCATION = '{snow_lat},{snow_lon}';

(function loadSnowAndMeteo(){{
  ...
  // ── Icones météo WMO (utilisé par Open-Meteo) ──
  function ico(c){{
    if(c==null) return '☁️';
    if(c===0) return '☀️';
    if(c<=3) return '🌤️';
  ...
  // ── 1. WorldWeatherOnline Ski API — enneigement réel par altitude ──
  var wwoUrl = 'https://api.worldweatheronline.com/premium/v1/ski.ashx'
    + '?key=' + WWO_KEY
    + '&q=' + WWO_LOCATION
    + '&num_of_days=1&tp=24&format=json&includeLocation=no&mktd=no';

  var wwoPromise = fetch(wwoUrl)
    .then(function(r){{ return r.json(); }})
    .then(function(d){{
      var wr = d && d.data && d.data.weather && d.data.weather[0];
      if(!wr) return null;
      var hourly = wr.hourly && wr.hourly[0]; // données du jour
      var monthly = d.data.ClimateAverages && d.data.ClimateAverages[0] && d.data.ClimateAverages[0].month;
      var curMonth = new Date().getMonth(); // 0-11

      // Neige actuelle en cm — WWO donne top/mid/bottom en cm
      var topSnow = parseInt((wr.top_snow_cm||'').replace(/[^0-9]/g,'')) || 0;
      var midSnow = parseInt((wr.mid_snow_cm||'').replace(/[^0-9]/g,'')) || 0;
      var botSnow = parseInt((wr.bottom_snow_cm||'').replace(/[^0-9]/g,'')) || 0;

      // Moyennes historiques mensuelles (WWO ClimateAverages)
      var avgTopSnow = 0, avgBotSnow = 0;
      if(monthly && monthly[curMonth]){{
        avgTopSnow = Math.round(parseFloat(monthly[curMonth].top_snow_cm||0));
        avgBotSnow = Math.round(parseFloat(monthly[curMonth].bottom_snow_cm||0));
      }}

      return {{ topSnow:topSnow, midSnow:midSnow, botSnow:botSnow, avgTopSnow:avgTopSnow, avgBotSnow:avgBotSnow }};
    }})
    .catch(function(){{ return null; }});

  // ── 2. Open-Meteo — météo 5 jours + chutes de neige prévues ──
  ...
  var omPromise = fetch(omUrl)
    .then(function(r){{ return r.json(); }})
    .catch(function(){{ return null; }});

  // ── Attendre les deux appels ──
  Promise.all([wwoPromise, omPromise]).then(function(results){{
    var wwo = results[0];
    var om  = results[1];

    // ── Onglet MÉTÉO — alimenté par Open-Meteo ──
    var ld = document.getElementById('meteoLoading');
    if(ld) ld.style.display='none';

    if(om && om.current){{
      var nowEl = document.getElementById('meteoNow');
      document.getElementById('meteoNowTemp').textContent = Math.round(om.current.temperature_2m)+'°';
      document.getElementById('meteoNowIco').textContent  = ico(om.current.weathercode);
      var sub = om.current.wind_speed_10m ? 'Vent '+Math.round(om.current.wind_speed_10m)+' km/h' : '';
      // Si WWO a fourni la neige au village on l'affiche ici aussi
      if(wwo && wwo.botSnow > 0) sub = '❄️ Village : '+wwo.botSnow+' cm';
    ...
    if(om && om.daily && om.daily.time){{
      ...
      for(var i=0; i<om.daily.time.length; i++){{
        var dt  = new Date(om.daily.time[i]+'T12:00:00');
        var lbl = i===0 ? 'Auj.' : DAY_LBL[dt.getDay()];
        var tmax = Math.round(om.daily.temperature_2m_max[i]);
        var tmin = Math.round(om.daily.temperature_2m_min[i]);
        var sf   = Math.round(om.daily.snowfall_sum[i]||0);
        h += '<div class="meteo-day">'
           + '<div class="meteo-day-lbl">'+lbl+'</div>'
           + '<div class="meteo-day-ico">'+ico(om.daily.weathercode[i])+'</div>'
           + '<div class="meteo-day-t">'+tmax+'° / '+tmin+'°</div>'
           + (sf>0 ? '<div class="meteo-day-snow">❄ '+sf+'cm</div>' : '')
           + '</div>';
      }}
      if(g){{ g.innerHTML=h; g.style.display='grid'; }}
    }}

    // ── Onglet ENNEIGEMENT — alimenté par WWO ──
    if(wwo){{
      // Village (bottom) actuel vs moyenne historique
      var vilEl  = document.getElementById('snowVillageVal');
      var vilAvg = document.getElementById('snowVillageAvg');
      var somEl  = document.getElementById('snowSommetVal');
      var somAvg = document.getElementById('snowSommetAvg');
      if(vilEl)  vilEl.textContent  = wwo.botSnow;
      if(vilAvg) vilAvg.textContent = wwo.avgBotSnow > 0 ? wwo.avgBotSnow : '—';
      if(somEl)  somEl.textContent  = wwo.topSnow;
      if(somAvg) somAvg.textContent = wwo.avgTopSnow > 0 ? wwo.avgTopSnow : '—';

      // Barre de comparaison village
      ...
      if(barWrap && barFill && barLabel && wwo.avgBotSnow > 0){{
        barWrap.style.display = 'block';
        var pct = Math.min(150, Math.round((wwo.botSnow / wwo.avgBotSnow) * 100));
        barFill.style.width      = Math.min(100,pct)+'%';
        barFill.style.background = pct>=90 ? '#2ea84e' : pct>=60 ? '#f0a500' : '#cc2200';
        var txt = pct>=110 ? 'Village : au-dessus de la normale ('+pct+'%)'
                : pct>=80  ? 'Village : dans la normale ('+pct+'%)'
                           : 'Village : en dessous de la normale ('+pct+'%)';
        barLabel.textContent = txt;
      }} else if(barWrap && wwo.avgBotSnow === 0){{
        barWrap.style.display = 'block';
      ...
      // Source tag
      var srcEl = document.getElementById('snowSourceTag');
      if(srcEl) srcEl.textContent = 'Source : WorldWeatherOnline Ski API';
    }} else {{
      // Fallback si WWO échoue : masquer les valeurs
      var errEls = ['snowVillageVal','snowVillageAvg','snowSommetVal','snowSommetAvg'];
      errEls.forEach(function(id){{ var el=document.getElementById(id); if(el) el.textContent='—'; }});
      var srcEl = document.getElementById('snowSourceTag');
      if(srcEl) srcEl.textContent = 'Données temporairement indisponibles';
    }}

    // ── Barres chutes prévues (Open-Meteo) ──
    if(om && om.daily && om.daily.snowfall_sum){{
      var barsEl = document.getElementById('snowForecastBars');
      var lblsEl = document.getElementById('snowForecastLabels');
      if(barsEl && lblsEl){{
        var sfArr = om.daily.snowfall_sum.map(function(v){{ return Math.round(v||0); }});
  ...
  <div class="main-grid">
    ...
    <div>
      ...
      <div class="tab-content" id="tab-neige">
        ...
        <!-- Lecture principale : village + sommet, actuel vs moyenne historique -->
        <div style="background:linear-gradient(135deg,#e8f3fb,#d0e7f5);border-radius:14px;padding:20px;margin-bottom:14px">
          <div style="font-size:.68rem;font-weight:800;text-transform:uppercase;letter-spacing:.07em;color:#3a7db8;margin-bottom:14px">❄️ Enneigement en ce moment</div>

          <!-- Village -->
          <div style="margin-bottom:14px">
            <div style="font-size:.7rem;font-weight:700;color:#1a5a8a;margin-bottom:8px;display:flex;align-items:center;gap:6px">
              🏘 Village · {s['alt_min']} m
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
              <div style="background:white;border-radius:10px;padding:12px;text-align:center;border:2px solid #3a7db8">
                <div style="font-size:.6rem;font-weight:700;color:var(--text-light);text-transform:uppercase;margin-bottom:5px">Maintenant</div>
                <div id="snowVillageVal" style="font-family:'DM Serif Display',serif;font-size:2rem;color:#1a5a8a;line-height:1">—</div>
                <div style="font-size:.65rem;color:#3a7db8;font-weight:600;margin-top:2px">cm</div>
              </div>
              <div style="background:rgba(255,255,255,.65);border-radius:10px;padding:12px;text-align:center">
                <div style="font-size:.6rem;font-weight:700;color:var(--text-light);text-transform:uppercase;margin-bottom:5px">Moy. ce mois</div>
                <div id="snowVillageAvg" style="font-family:'DM Serif Display',serif;font-size:2rem;color:#2a6090;line-height:1">—</div>
                <div style="font-size:.65rem;color:#3a7db8;font-weight:600;margin-top:2px">cm</div>
              </div>
            </div>
          </div>
          ...
          <div style="margin-bottom:14px">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
              <div style="background:white;border-radius:10px;padding:12px;text-align:center;border:2px solid #1a5a8a">
                <div style="font-size:.6rem;font-weight:700;color:var(--text-light);text-transform:uppercase;margin-bottom:5px">Maintenant</div>
                <div id="snowSommetVal" style="font-family:'DM Serif Display',serif;font-size:2rem;color:#1a5a8a;line-height:1">—</div>
                <div style="font-size:.65rem;color:#3a7db8;font-weight:600;margin-top:2px">cm</div>
              </div>
              <div style="background:rgba(255,255,255,.65);border-radius:10px;padding:12px;text-align:center">
                <div style="font-size:.6rem;font-weight:700;color:var(--text-light);text-transform:uppercase;margin-bottom:5px">Moy. ce mois</div>
                <div id="snowSommetAvg" style="font-family:'DM Serif Display',serif;font-size:2rem;color:#2a6090;line-height:1">—</div>
                <div style="font-size:.65rem;color:#3a7db8;font-weight:600;margin-top:2px">cm</div>
              </div>
            </div>
          </div>

          <!-- Barre de comparaison village actuel vs moy. -->
          <div id="snowCompareBar" style="display:none">
            <div style="font-size:.65rem;color:#3a7db8;font-weight:700;margin-bottom:6px">Comparaison vs normale du mois</div>
          ...
          <div id="snowSourceTag" style="font-size:.6rem;color:rgba(26,90,138,.6);margin-top:10px;text-align:right">Chargement…</div>
        </div>

        <!-- Village vs Sommet : données WWO en temps réel -->
        <div style="background:white;border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:14px">
          <div style="font-size:.68rem;font-weight:800;text-transform:uppercase;letter-spacing:.07em;color:var(--text-light);margin-bottom:12px">📍 Contexte altitude</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
            <div style="padding:12px;background:#f8f9ff;border-radius:10px">
              <div style="font-size:.72rem;font-weight:700;color:var(--text);margin-bottom:4px">🏘 Village · {s['alt_min']} m</div>
            ...
            <div style="padding:12px;background:#e8f3fb;border-radius:10px">
              <div style="font-size:.72rem;font-weight:700;color:#1a5a8a;margin-bottom:4px">⛰ Sommet · {s['alt_max']} m</div>
              <div style="font-size:.8rem;color:var(--text-mid);line-height:1.5">
        ...
        <div style="background:linear-gradient(135deg,#f8f5f0,#f0e8da);border:1px solid var(--wood-pale);border-radius:12px;padding:16px">
          ...
          <p style="font-size:.68rem;color:var(--text-light);margin-top:10px;line-height:1.5">{snow_reliability} Données enneigement : WorldWeatherOnline Ski API (mesures par niveau d'altitude, actualisées 4x/jour). Prévisions chutes : Open-Meteo.</p>
        </div>
