...
DOMAINES = {
    "portes-du-soleil": {
        ...
        "desc": "Le plus grand domaine skiable transfrontalier au monde, 12 stations françaises et suisses reliées skis aux pieds autour du Chablais.",
    },
    "3-vallees": {
        ...
        "km_propre": {"Val Thorens":150,"Les Menuires":160,"Méribel":160,"Courchevel":150,"Saint-Martin-de-Belleville":40,"La Tania":25,"Orelle":35},
        "km_total": 600, "remontees_total": 170, "forfait_domaine": 82,
        "alt_min": 1100, "alt_max": 3230,
        "desc": "Le plus grand domaine skiable du monde. Huit stations reliées en Tarentaise et Maurienne, de Courchevel à Val Thorens.",
    },
    "espace-killy": {
        ...
        "desc": "Deux mythes du ski français reliés par le col de Fresse, domaine des Jeux Olympiques d'Albertville 1992.",
    },
    "paradiski": {
        "name": "Paradiski", "massif": "Alpes du Nord", "pays": ["France"],
        "stations": ["La Plagne","Les Arcs","Peisey-Vallandry","Montchavin-Les Coches"],
        "km_propre": {"La Plagne":225,"Les Arcs":200,"Peisey-Vallandry":60},
        "km_total": 425, "remontees_total": 160, "forfait_domaine": 68,
        "alt_min": 1200, "alt_max": 3250,
        "desc": "La Plagne et Les Arcs, reliées depuis 2003 par le téléphérique Vanoise Express, l'un des plus grands du monde.",
    },
    "grand-massif": {
        "name": "Le Grand Massif", "massif": "Alpes du Nord", "pays": ["France"],
        "stations": ["Flaine","Les Carroz","Morillon","Samoëns","Sixt-Fer-à-Cheval"],
        "km_propre": {"Flaine":145,"Samoëns":40,"Les Carroz":45,"Morillon":35},
        "km_total": 265, "remontees_total": 62, "forfait_domaine": 51,
        "alt_min": 700, "alt_max": 2500,
        "desc": "Cinq villages authentiques du Faucigny reliés autour du plateau des Grandes Platières.",
    },
    "sybelles": {
        ...
        "desc": "Six stations familiales de Maurienne reliées en un seul domaine autour des 3 Lacs.",
    },
    "espace-diamant": {
        ...
        "desc": "Le plus jeune grand domaine français (2005), entre Beaufortain et Val d'Arly.",
    },
    "evasion-mont-blanc": {
        ...
        "km_propre": {"Megève":230,"Saint-Gervais":45,"Les Contamines-Montjoie":120,"Combloux":60},
        "km_total": 263, "remontees_total": 107, "forfait_domaine": 50,
        "alt_min": 850, "alt_max": 2353,
        "desc": "Megève et ses villages voisins, au pied du Mont-Blanc — 263 km réellement reliés par remontées.",
    },
    "espace-san-bernardo": {
        ...
        "desc": "La Rosière reliée à La Thuile (Italie) par le col du Petit-Saint-Bernard — seul domaine franco-italien des Alpes du Nord.",
    },
    "galibier-thabor": {
        ...
        "desc": "Valloire et Valmeinier reliées au pied du col du Galibier.",
    },
    "alpe-dhuez-grand-domaine": {
        ...
        "desc": "L'Île au Soleil, 300 jours de soleil par an, et la Sarenne — la piste noire la plus longue d'Europe.",
    },
    "serre-chevalier-vallee": {
        ...
        "desc": "Le plus grand domaine skiable des Alpes du Sud, quatre villages intégrés en une seule station.",
    },
    "voie-lactee": {
        ...
        "desc": "Montgenèvre reliée à cinq stations italiennes du Piémont (Sestrières, Sauze d'Oulx...).",
    },
    "foret-blanche": {
        ...
        "desc": "Vars et Risoul, reliées par le col de Valbelle, 2e plus grand domaine des Alpes du Sud.",
    },
    "grand-tourmalet": {
        ...
        "desc": "Le plus grand domaine skiable des Pyrénées françaises, réunissant Barèges et La Mongie de part et d'autre du col du Tourmalet.",
    },
    "grand-domaine-valmorel": {
        "name": "Le Grand Domaine", "massif": "Alpes du Nord", "pays": ["France"],
        "stations": ["Saint-François-Longchamp"],
        "km_propre": {"Saint-François-Longchamp": 70},
        "km_total": 165, "remontees_total": 47, "forfait_domaine": 58,
        "alt_min": 1200, "alt_max": 2550,
        "desc": "Valmorel et Saint-François-Longchamp reliées par le col de la Madeleine.",
    },
    "le-devoluy": {
        ...
        "desc": "Superdévoluy et La Joue du Loup, deux villages skis-aux-pieds reliés au cœur du Dévoluy.",
    },
    "grand-sancy": {
        ...
        "desc": "Liaison par les crêtes entre Super Besse et Le Mont-Dore — ouverte selon l'enneigement uniquement.",
        "conditionnel": True,
    },
}
...
{carousel_dots_html}
    ...
    if domaine:
        display_km = domaine.get('km_propre', {}).get(s['name'], s['km'])
        display_alt_min = domaine.get('alt_village', {}).get(s['name'], s['alt_min'])
        km_lbl = "Pistes skiables" if s['name'] in domaine.get('km_propre', {}) else "Pistes (secteur)"
    else:
        display_km = s['km']
...
<head>
  ...
  <meta name="description" content="{s['name']} : {display_km} km de pistes, altitude {display_alt_min}-{s['alt_max']}m, forfait {s['forfait']}€/jour. {s.get('desc','')[:100]}">
  ...
  <meta property="og:description" content="{display_km} km · {s['alt_max']}m · {s['forfait']}€/j · {s['massif']}">
  ...
  <style>
    *{{box-sizing:border-box;margin:0;padding:0}}
    ...
    @media(min-width:1200px){{
      ...
      .main-grid{{grid-template-columns:1fr 400px;gap:28px}}
    ...
    /* ONGLETS */
    .tab-nav-wrap{{position:relative;border-radius:14px 14px 0 0;background:var(--wood-pale)}}
    .tab-nav{{display:flex;gap:6px;margin-bottom:0;flex-wrap:nowrap;overflow-x:auto;-webkit-overflow-scrolling:touch;scrollbar-width:none;background:var(--wood-pale);border-radius:14px 14px 0 0;padding:10px 10px 0;border:1px solid var(--border);border-bottom:none}}
    ...
    .domaine-box{{
      ...
      background:linear-gradient(145deg,#fff8ec 0%,#fdecc8 100%);border:1.5px solid #f0c869;
      box-shadow:0 4px 18px rgba(212,155,0,.15);transition:transform .15s,box-shadow .15s
    }}
    .domaine-box:hover{{transform:translateY(-2px);box-shadow:0 8px 26px rgba(212,155,0,.25)}}
    .domaine-box-head{{padding:16px 20px 10px;display:flex;flex-direction:column;gap:3px}}
    .domaine-box-tag{{font-size:.7rem;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:#a0720a}}
    .domaine-box-name{{font-family:"DM Serif Display",serif;font-size:1.35rem;color:#5c3d00;display:flex;align-items:center;gap:8px}}
    .domaine-box-arrow{{font-size:1.1rem;color:#a0720a}}
    .domaine-box-stats{{display:grid;grid-template-columns:repeat(4,1fr);gap:0;padding:0 12px;border-top:1px solid rgba(160,114,10,.15);border-bottom:1px solid rgba(160,114,10,.15)}}
    .domaine-box-stats>div{{text-align:center;padding:12px 4px;min-width:0}}
    .dbs-val{{font-family:"DM Serif Display",serif;font-size:1.05rem;color:#5c3d00;line-height:1.15;word-break:break-word;overflow-wrap:break-word}}
    .dbs-lbl{{font-size:.6rem;color:#a0720a;text-transform:uppercase;letter-spacing:.03em;margin-top:3px}}
    @media(max-width:600px){{.domaine-box-stats{{grid-template-columns:repeat(2,1fr)}}}}
    .domaine-box-foot{{padding:10px 20px;font-size:.76rem;color:#a0720a;font-weight:600}}

    /* Onglet Domaine */
    .dt-hero{{background:linear-gradient(145deg,#fff8ec 0%,#fdecc8 100%);border:1.5px solid #f0c869;border-radius:14px;padding:16px 18px;margin-bottom:16px}}
    .dt-hero h3{{font-family:"DM Serif Display",serif;font-size:1.3rem;color:#5c3d00;margin-bottom:6px}}
    .dt-hero p{{color:#8a6512;font-size:.88rem;line-height:1.6}}
    ...
    .domaine-soeurs-grid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(170px,1fr));gap:12px}}
    .domaine-soeur-card{{
  ...
  <div class="main-grid">
    ...
    <div>
      ...
      <div class="tab-content active" id="tab-infos">
        ...
        <!-- PISTES -->
        <div class="section">
          <div class="section-title">Domaine skiable — {display_km} km · {display_alt_min}m à {s['alt_max']}m</div>
          <div class="piste-row">
        ...
        <div class="section" style="margin-bottom:0">
          ...
          <p style="font-size:.88rem;line-height:1.72;color:var(--text-mid)">
            ...
            <strong>{display_km} km de pistes</strong> entre {display_alt_min} m et {s['alt_max']} m,
...
def render_domaine_page(slug, d):
    ...
    conditionnel_html = (
        '<div class="dm-warn">⚠️ La liaison entre les stations de ce domaine n\'est ouverte que lorsque l\'enneigement le permet — vérifiez son statut avant de partir.</div>'
        if d.get('conditionnel') else ''
    )
    stations_cards = []
    for name in d['stations']:
        st = next((x for x in DATA if x['name'] == name), None)
        if not st:
            continue
        st_slug = slugify(name)
        km_prop = d.get('km_propre', {}).get(name)
        km_line = f"{km_prop} km propres" if km_prop else "km propres non détaillés"
        stations_cards.append(f'''<a href="../stations/{st_slug}.html" class="dm-station-card">
      <div class="dm-station-name">{name}</div>
      <div class="dm-station-meta">🏘 {st.get('alt_min','?')}m · ⛰ {st.get('alt_max','?')}m</div>
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no">
<title>{d['name']} — Domaine skiable {d['km_total']} km | SnowFinder</title>
<meta name="description" content="{d['name']} : {d['km_total']} km de pistes reliées, {len(d['stations'])} stations — {', '.join(d['stations'])}. Forfait, altitude, remontées.">
<link rel="canonical" href="{canonical}">
...
<style>
  :root{{--wood:#2d5a8a;--wood-light:#e8f0f8;--wood-pale:#f0f6fb;--text:#1a2942;--text-mid:#5a6a80;--text-light:#8a97a8;--navy:#0d3a6e}}
  *{{box-sizing:border-box;margin:0;padding:0}}
  html,body{{touch-action:pan-x pan-y;overflow-x:hidden;max-width:100%}}
  body{{font-family:"DM Sans",sans-serif;color:var(--text);background:linear-gradient(135deg,#e8f3fb 0%,#d0e7f5 50%,#e8f3fb 100%);background-attachment:fixed;min-height:100vh;padding-bottom:66px}}
  a{{color:inherit}}
  .dm-hero{{background:linear-gradient(135deg,var(--navy) 0%,#1a5a8a 100%);color:white;padding:38px 20px 30px;text-align:center;position:relative}}
  .dm-hero-massif{{display:inline-block;background:rgba(255,255,255,.16);border:1px solid rgba(255,255,255,.3);border-radius:20px;padding:5px 14px;font-size:.75rem;font-weight:700;margin-bottom:12px}}
  .dm-hero h1{{font-family:"DM Serif Display",serif;font-size:2.1rem;margin-bottom:8px}}
  .dm-hero p{{opacity:.85;font-size:.92rem;max-width:520px;margin:0 auto}}
  .dm-close{{position:absolute;top:16px;right:16px;width:38px;height:38px;border-radius:50%;background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.3);color:white;font-size:1rem;display:flex;align-items:center;justify-content:center;text-decoration:none}}
  .dm-breadcrumb{{max-width:900px;margin:14px auto 0;padding:0 20px;font-size:.78rem;color:var(--text-mid)}}
  .dm-breadcrumb a{{text-decoration:none;color:#1a5a8a;font-weight:600}}
  .dm-flag,.dm-warn{{max-width:900px;margin:14px auto;padding:10px 16px;border-radius:10px;font-size:.82rem;font-weight:600}}
  .dm-flag{{background:#eaf3ff;color:#0d3a6e}}
  .dm-warn{{background:#fff4e6;color:#a05a2c}}
  .dm-stats{{max-width:900px;margin:18px auto;padding:0 20px;display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:12px}}
  .dm-stat{{background:white;border-radius:14px;padding:16px 10px;text-align:center;box-shadow:0 2px 10px rgba(0,0,0,.05)}}
  .dm-stat-val{{font-family:"DM Serif Display",serif;font-size:1.5rem;color:var(--navy)}}
  .dm-stat-lbl{{font-size:.7rem;color:var(--text-mid);text-transform:uppercase;letter-spacing:.05em;margin-top:2px}}
  .dm-section{{max-width:900px;margin:24px auto;padding:0 20px}}
  .dm-section-title{{font-family:"DM Serif Display",serif;font-size:1.3rem;margin-bottom:14px}}
  .dm-desc{{background:white;border-radius:14px;padding:18px 20px;font-size:.92rem;line-height:1.7;color:var(--text-mid);box-shadow:0 2px 10px rgba(0,0,0,.05)}}
  .dm-stations-grid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:14px}}
  .dm-station-card{{display:block;background:white;border-radius:14px;padding:16px 18px;text-decoration:none;box-shadow:0 2px 10px rgba(0,0,0,.05);transition:transform .15s,box-shadow .15s}}
  .dm-station-card:hover{{transform:translateY(-3px);box-shadow:0 8px 20px rgba(0,0,0,.1)}}
  .dm-station-name{{font-weight:800;font-size:1.05rem;color:var(--text);margin-bottom:4px}}
  .dm-station-meta{{font-size:.78rem;color:var(--text-mid);margin-bottom:6px}}
  .dm-station-km{{font-size:.75rem;color:#1a5a8a;font-weight:600}}
  .dm-station-cta{{margin-top:8px;font-size:.8rem;font-weight:700;color:var(--navy)}}
...
<div class="dm-hero">
  <a href="../recherche.html" class="dm-close" aria-label="Retour">✕</a>
