...
{carousel_dots_html}
    ...
    if s['alt_max'] >= 2000:
        snow_reliability = "Au-dessus de 2 000 m, la neige est g\u00e9n\u00e9ralement garantie de mi-d\u00e9cembre \u00e0 fin mars."
    elif s['alt_max'] >= 1500:
        snow_reliability = "A cette altitude, l'enneigement d\u00e9pend des conditions de l'ann\u00e9e. V\u00e9rifiez l'\u00e9tat des pistes avant de partir."
    else:
        snow_reliability = "L'altitude mod\u00e9r\u00e9e rend l'enneigement sensible aux variations climatiques. Privil\u00e9giez les mois de janvier-f\u00e9vrier."
...
(function loadMeteo(){{
  ...
  fetch(url).then(function(r){{return r.json();}}).then(function(d){{
    ...
    if(d.current){{
      ...
      // Mettre à jour l'onglet Enneigement
      var sdEl=document.getElementById('snowDepthVal');
      if(sdEl){{
        if(sd!=null&&sd>0) sdEl.textContent=Math.round(sd*100)+' cm';
        else sdEl.textContent='Pas de neige détectée actuellement';
        sdEl.style.color='#1a5a8a';
      }}
    }}
  ...
  <div class="main-grid">
    ...
    <div>
      ...
      <div class="tab-content active" id="tab-infos">
        ...
        <!-- BOOKING / EXPEDIA EN HAUT -->
        <div style="margin-bottom:20px">
          <div style="font-family:'DM Serif Display',serif;font-size:1rem;color:var(--text-mid);margin-bottom:10px;padding-bottom:8px;border-bottom:2px solid var(--wood-pale)">🏨 Réserver un hébergement à {s['name']}</div>
          <div class="be-cards">
            <a href="{booking_url}" target="_blank" rel="noopener sponsored" class="be-card">
              <img src="../img/chalet-booking.jpg" alt="Chalet Booking" class="be-card-img" onerror="this.src='https://images.unsplash.com/photo-1502784444187-359ac186c5bb?w=400&q=80'">
              <div class="be-card-body">
                <div class="be-card-brand">Booking.com</div>
                <div class="be-card-txt">Voir les hébergements à {s['name']} → Dès ~{prix_nuit}€/nuit</div>
              </div>
            </a>
            <a href="{expedia_url}" target="_blank" rel="noopener sponsored" class="be-card">
              <img src="../img/chalet-expedia.jpg" alt="Chalet Expedia" class="be-card-img" onerror="this.src='https://images.unsplash.com/photo-1520250497591-112f2f40a3f4?w=400&q=80'">
              <div class="be-card-body">
                <div class="be-card-brand">Expedia</div>
                <div class="be-card-txt">Comparer les prix à {s['name']} → Voir les offres</div>
              </div>
            </a>
          </div>
        </div>
      ...
      <div class="tab-content" id="tab-neige">
        ...
        <!-- Enneigement de référence -->
        <div class="section" style="background:linear-gradient(135deg,#e8f3fb,#d0e7f5);border-radius:12px;border:none">
          <div class="section-title">❄️ Enneigement moyen de référence</div>
          <div style="display:flex;align-items:flex-end;gap:6px;margin-bottom:8px">
            <div style="font-family:'DM Serif Display',serif;font-size:3rem;color:#1a5a8a;line-height:1">{s.get('snow', '—')}</div>
            <div style="font-size:.9rem;color:#3a7db8;font-weight:700;padding-bottom:8px">cm</div>
          </div>
          <p style="font-size:.82rem;color:var(--text-mid);line-height:1.6">Hauteur de neige moyenne en pleine saison (décembre–février) basée sur les données historiques. Indicatif.</p>
        </div>

        <!-- Altitude et garantie neige -->
        <div class="section" style="margin-bottom:0">
          <div class="section-title">Fiabilité de l'enneigement</div>
          <p style="font-size:.88rem;line-height:1.72;color:var(--text-mid)">
            {s['name']} est situé entre <strong>{s['alt_min']} m</strong> (village) et
            <strong>{s['alt_max']} m</strong> (sommet).
            {snow_reliability}
          </p>
          <div id="snowDepthLive" style="margin-top:14px;padding:12px 16px;background:#fff;border-radius:10px;border:1px solid var(--border)">
            <div style="font-size:.72rem;font-weight:700;color:var(--text-light);text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px">Neige au sol en ce moment</div>
            <div id="snowDepthVal" style="font-family:'DM Serif Display',serif;font-size:1.6rem;color:#1a5a8a">Chargement…</div>
          </div>
        </div>
