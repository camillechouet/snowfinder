...
Logique :
  - Saison (nov-avril) : neige (lundi/jeudi, max 2/sem) + vacances scolaires (J-14/J-7)
  - Hors saison (mai-oct) : 1 message humoristique par semaine (lundi) pour garder le lien
"""
...
MSG_NEIGE = [
    ...
    ("{snow}cm de neige fraîche à {name}. La montagne t'appelle 🎿",
...
MSG_ENNEI = [
    ...
    ("La montagne est belle en ce moment. Vraiment belle 🏔️",
    ...
    ("Honnêtement, les conditions sont vraiment bonnes en ce moment 😌🎿",
     "Beau soleil + pistes bien enneigées dans les {massif} ☀️❄️"),
    ("Les pistes des {massif} sont au top cette semaine 🎿",
    ...
    ("Cette semaine dans les {massif} : pistes en parfait état ❄️ On valide.",
     "Conditions quasi parfaites dans les {massif}. Juste parfaites 🎿"),
]
...
MSG_ETE = [
    ...
    ("Si tu es vraiment en manque de fondue savoyarde, rien ne t'empêche de partir en montagne cet été 🏔️",
     "Petit rappel : la montagne c'est bien aussi en été. Randos, lacs, fraîcheur 🥾"),
    ...
    ("Pendant ce temps, à 2000m d'altitude, on prépare déjà ton hiver 🏔️",
     "Chaque jour qui passe nous rapproche un peu plus de la neige. Tiens bon 💪❄️"),
    ("Un jour sans ski est un jour presque perdu. Encore quelques mois à tenir 😅",
     "Les télésièges se reposent, toi aussi profite de l'été... mais pas trop 😉"),
    ("Petit calcul : il reste moins de jours avant l'hiver que de jours écoulés depuis la fermeture des pistes. Ça va vite !",
     "Le compte à rebours vers la neige a officiellement commencé quelque part dans nos têtes 🕐❄️"),
    ("Cet été, va prendre l'air en altitude — la montagne n'attend pas décembre pour être belle 🥾🏔️",
     "Chalets, alpages, silence... la montagne d'été a aussi son charme 🌿"),
    ("Un skieur ne meurt jamais, il hiberne juste jusqu'à décembre ❄️😴",
     "On ne dira rien si tu regardes déjà les prévisions neige de novembre 👀"),
    ("La saison estivale, c'est la trêve. La vraie vie reprend à la première chute de neige ❄️",
     "Prépare ton matos, l'hiver approche plus vite qu'on croit 🎿"),
]
...
def main():
    ...
    print(f"=== SnowFinder — {today} (jour {weekday}) — {'saison' if en_saison else 'hors saison'} ===")
    ...
    if not en_saison:
        ...
        if weekday == 0 and semaine_paire and notifs_envoyees == 0:
            titre, corps = random.choice(MSG_ETE)
            print(f"\n☀️ Message hors-saison (lundi, semaine paire)")
            n = send_notif(titre, corps, "https://snowfinder.fr/recherche.html", "off_season")
            print(f"✅ Notif été envoyée → {n} abonné(s)")
        else:
            print(f"\nHors saison — pas le bon lundi (1x/2 semaines) ou déjà 1 notif.")
        return

    # ── 3. NEIGE / ENNEIGEMENT — lundi (0) et jeudi (3) uniquement ───────────
    ...
    if notifs_envoyees >= limit:
        print(f"\nLimite de {limit} notifs atteinte.")
        return
    ...
    if top_neige:
        ...
        n = send_notif(titre, corps, "https://snowfinder.fr/enneigement.html", "enneigement_top")
        print(f"✅ Notif enneigement envoyée → {n} abonné(s)")
    else:
        print("\nPas de neige significative — pas de notif.")

    print(f"\n=== Total : {notifs_envoyees} notif(s) ===")
