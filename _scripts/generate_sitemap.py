...
def build_sitemap(stations: list[dict]) -> tuple[str, list[str], list[str]]:
    ...
    # Pages comparatifs (auto-découvertes dans le dossier /comparatifs/)
    comparatifs_dir = REPO_ROOT / "comparatifs"
    nb_comparatifs = 0
    if comparatifs_dir.exists():
        # Page d'accueil des comparatifs
        if (comparatifs_dir / "index.html").exists():
            lines.append(url_entry("/comparatifs/", today, "weekly", "0.7"))
            nb_comparatifs += 1
        # Pages comparatifs individuelles
        for f in sorted(comparatifs_dir.glob("*.html")):
            if f.name == "index.html":
                continue
            lines.append(url_entry(f"/comparatifs/{f.name}", today, "weekly", "0.8"))
            nb_comparatifs += 1

    lines.append("</urlset>")
    lines.append("")  # newline final

    return "\n".join(lines), slugs_ok, slugs_manquants, nb_comparatifs
...
def main() -> None:
    ...
    xml, slugs_ok, slugs_manquants, nb_comparatifs = build_sitemap(stations)
    SITEMAP_OUT.write_text(xml, encoding="utf-8")

    print(f"✓ {len(slugs_ok)} pages stations ajoutées au sitemap")
    print(f"✓ {nb_comparatifs} pages comparatifs ajoutées au sitemap")
